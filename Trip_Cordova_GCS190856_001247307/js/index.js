...
let db = window.openDatabase('Trip', '1.0', 'Trip', 20000);
...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #type').text($('#page-create #frm-register #type').val());
    $('#page-create #frm-confirm #people').text($('#page-create #frm-register #people').val());
...
function registerTrip(e) {
    ...
    let people = $('#page-create #frm-register #people').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Type, People, Risk, Date) VALUES (?, ?, ?, ?, ?, ?, julianday('${date}'))`;
        console.log(query);
        tx.executeSql(query, [name, destination, description, type, people, risk], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #type').text(result.rows[0].Type);
                $('#page-detail #detail #people').text(result.rows[0].People);
...
function addExpense(e) {
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            let message = `Added expense '${amount.toLocaleString("en-US")} $ (${type})'.`;
                    ...
                    <div class='list'>
                        <table style='white-space: nowrap; width: 100%;'>
                            <tr style='height: 25px;'>
                                <td>${expense.Type}: ${expense.Amount.toLocaleString('en-US')} $</td>
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #type`).val(result.rows[0].Type);
                $(`#page-detail #frm-update #people`).val(result.rows[0].People);
...
function updateTrip(e) {
    ...
    let people = $('#page-detail #frm-update #people').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?, Risk = ?, Type = ?, People = ?,Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name, destination, description, risk, type, people, id], transactionSuccess, transactionError);
    ...
    for (let trip of list) {
        tripList +=
            `<li><a data-details='{"Id" : ${trip.Id}}'>
                ...
                <p><small>${trip.DateConverted} - <small>Type: ${trip.Type} - People: ${trip.People}</small></small></p>
            </a></li>`;
    }
    tripList += `</ul>`;
